...
#include <cassert>
#include <cmath>
#include <vector>				//real vector class... 
...
#include "transform.h"			// utility functions for mocapvector and matrix transformation  
#include "display.h"   
...
static vector<Posture> keyframes;
static char actorfile[50];
static char motionfile[50];
float tau = 0.5;
/***************  Functions *******************/
...
mocapvector cmr_vec(float i, mocapvector v1, mocapvector v2, mocapvector v3, mocapvector v4) {
	float i_a = i*i;
	float i_b = i_a*i;
	float M1 = -tau*i_b + 2 * tau*i_a - tau*i;
	float M2 = (2 - tau)*i_b + (tau - 3)*i_a + 1;
	float M3 = (tau - 2)*i_b + (3 - 2 * tau)*i_a + tau*i;
	float M4 = tau*i_b - tau*i_a;
...
{
	if (keyframes.size() >= 4) {
		//need 4 frames for CMR interpolation
		keyframes.push_back(keyframes.back()); // copy final posture and insert at end of vector
		keyframes.insert(keyframes.begin(), keyframes.front());	//copy first posture and insert at beginning of vector

		pInterpMotion = new Motion(pSampledMotion->m_NumFrames);
		for (int i = 0; i < keyframes.size() - 3; ++i) {
			pInterpMotion->SetPosture(keyframes[i + 1].frame, keyframes[i + 1]);
			for (int j = keyframes[i + 1].frame + 1; j < keyframes[i + 2].frame; ++j) {
				Posture interpolate;
				float cmr_i = (float)(j - keyframes[i + 1].frame) / (keyframes[i + 2].frame - keyframes[i + 1].frame);
				interpolate.root_pos = cmr_vec(cmr_i, keyframes[i].root_pos, keyframes[i + 1].root_pos, keyframes[i + 2].root_pos, keyframes[i + 3].root_pos);		//interpolate root

				for (int k = 0; k < MAX_BONES_IN_ASF_FILE; ++k) {
					interpolate.bone_length[k] = cmr_vec(cmr_i, keyframes[i].bone_length[k], keyframes[i + 1].bone_length[k], keyframes[i + 2].bone_length[k], keyframes[i + 3].bone_length[k]);
					interpolate.bone_rotation[k] = cmr_vec(cmr_i, keyframes[i].bone_rotation[k], keyframes[i + 1].bone_rotation[k], keyframes[i + 2].bone_rotation[k], keyframes[i + 3].bone_rotation[k]);
					interpolate.bone_translation[k] = cmr_vec(cmr_i, keyframes[i].bone_translation[k], keyframes[i + 1].bone_translation[k], keyframes[i + 2].bone_translation[k], keyframes[i + 3].bone_translation[k]);
				}
				pInterpMotion->SetPosture(j, interpolate);
			}

			if (i == keyframes.size() - 4) { pInterpMotion->SetPosture(keyframes[i + 2].frame, keyframes[i + 2]); }
		}

		//create actor for interpolated postures. Color green for easy viewing
		Skeleton* temp_actor = new Skeleton(actorfile, MOCAP_SCALE);
		temp_actor->setBasePosture();
		temp_actor->color.r = 0.0;
		temp_actor->color.g = 1.0;
		temp_actor->color.b = 0.2;
		displayer.loadActor(temp_actor);										//load the actor

		//load interpolated motions
...
void keyframe_capture_callback(Fl_Button *button, void *){
	cout << "Captured frame" << endl;

	pSampledMotion->GetPosture(nFrameNum)->frame = nFrameNum;		//assign frame number value to posture
	keyframes.push_back(*pSampledMotion->GetPosture(nFrameNum));	//push back keyframe posture into keyframe vector
	Posture* keyposture = pSampledMotion->GetPosture(nFrameNum);	//copy posture for instantiating new motion file

	//create actor for keyframe posture. Color blue for easy viewing.
	Skeleton* temp_actor = new Skeleton(actorfile, MOCAP_SCALE);
	temp_actor->setBasePosture();
	temp_actor->color.r = 0.2;
	temp_actor->color.g = 0.0;
	temp_actor->color.b = 1.0;
	displayer.loadActor(temp_actor);										//load the actor


	//create new motion and set all postures to keyframe posture
	Motion* temp_motion = new Motion(motionfile, MOCAP_SCALE, pActor);
	for (int i = 0; i < temp_motion->m_NumFrames; ++i) {
		temp_motion->SetPosture(i, *keyposture);
	}
	displayer.loadMotion(temp_motion);
}
...
void load_callback(Fl_Button *button, void *) 
{
	...
	if(button==loadActor_button)
	{
		...
		if(filename != NULL)
		{
			//copy filename for use later
			strcpy(actorfile, filename);
			//Remove old actor
			//if(pActor != NULL) 
			//	delete pActor; 
